...
import { decodeRequestUser, extractRequestIp, type SessionUser } from "./jwt.server.js";
import { readCookies } from "./session.server.js";
import { withTenantContext } from "../db/tenant-context.server.js";
import { applyRlsSetting, withRls } from "../db/rls.server.js";
import { loadEffectivePermissions } from "../permissions/permission-service.server.js";
import {
...
export async function runInRls<T>(
  session: ResolvedSession,
  work: (tx: Awaited<ReturnType<typeof withRls<T>>>) => Promise<T>,
): Promise<T> {
  if (session.organizationId === 0n) {
    return work(undefined as unknown as Awaited<ReturnType<typeof withRls<T>>>);
  }
  return withTenantContext(
    {
      organizationId: session.organizationId,
    ...
    async () =>
      withRls(session.organizationId, { bypass: session.bypassTenant }, (tx) =>
        work(tx as unknown as Awaited<ReturnType<typeof withRls<T>>>),
      ),
  );
}
