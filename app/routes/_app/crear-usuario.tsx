 ...
 * del tenant activo (RLS-scoped, via DI). Action invoca `adminService.createUser`
 * directamente — NO fetch HTTP.
 *
 * El componente AdminUserForm legacy se mantiene tal cual pero recibe los
 * datos por props desde useLoaderData (en lugar de hacer fetch interno).
 */
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  return new Response(
    JSON.stringify({ roles, departments, csrfToken: csrf.token, orgId: session.organizationId.toString() }),
    { status: 200, headers: { "content-type": "application/json", ...Object.fromEntries(headers) } },
  );
}
...
export async function action({ request }: ActionFunctionArgs): Promise<Response> {
  ...
  const userData = {
    organization_id: form.get("organization_id")?.toString() ?? session.organizationId.toString(),
    role_id: Number(form.get("role_id")),
    department_id: Number(form.get("department_id")),
    user_name: form.get("user_name")?.toString() ?? "",
  ...
  try {
    await runInTenant(session, async () => adminService.createUser(userData));
  } catch (err) {
...
type LoaderData = { roles: { id: number; name: string }[]; departments: { id: number; name: string }[]; csrfToken: string; orgId: string };
