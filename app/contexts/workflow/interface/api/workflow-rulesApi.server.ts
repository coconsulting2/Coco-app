...
// @ts-ignore — JS module
import * as workflowRulesEngine from "~/contexts/workflow/application/workflowRulesEngine.js";
...
const ROUTES: Array<{ method: string; pattern: RegExp; handler: (m: RegExpMatchArray, ctx: any) => Promise<unknown> }> = [
  { method: "GET", pattern: /^$/, handler: async (m, { session, body }) => workflowRulesEngine.listRules?.() },
  { method: "GET", pattern: /^(\d+)$/, handler: async (m, { session, body }) => workflowRulesEngine.getRule?.(Number(m[1])) },
  { method: "POST", pattern: /^$/, handler: async (m, { session, body }) => workflowRulesEngine.createRule?.(body) },
  { method: "PUT", pattern: /^(\d+)$/, handler: async (m, { session, body }) => workflowRulesEngine.updateRule?.(Number(m[1]), body) },
];

export async function dispatchWorkflowRulesApi({ request, subpath }: DispatchArgs): Promise<Response> {
  ...
  try {
    ...
    return jsonError(404, `Unknown workflow-rules endpoint: ${method} ${path}`, "UNKNOWN_ENDPOINT");
  } catch (err) {
