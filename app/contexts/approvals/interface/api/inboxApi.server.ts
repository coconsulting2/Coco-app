...
const ROUTES: Array<{ method: string; pattern: RegExp; perm: string | null; handler: (m: RegExpMatchArray, ctx: any) => Promise<unknown> | unknown }> = [
  { method: "GET", pattern: /^inbox$/, perm: "travel_request:authorize", handler: async (m, { session, body, url }) => Authorizer.getPendingAuthorizations(session.user.user_id, session.user.user_id) },
];

export async function dispatchInboxApi({ request, subpath }: DispatchArgs): Promise<Response> {
