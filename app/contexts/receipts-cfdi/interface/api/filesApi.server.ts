...
// @ts-ignore
import * as receiptFileService from "~/contexts/receipts-cfdi/application/receiptFileService.js";
...
const ROUTES: Array<{ method: string; pattern: RegExp; perm: string | null; handler: (m: RegExpMatchArray, ctx: any) => Promise<unknown> | unknown }> = [
  { method: "POST", pattern: /^upload-receipt-files\/(\d+)$/, perm: null, handler: async (m, { session, body, url }) => { status: 501, message: 'multipart upload requires Node adapter — see receiptFileService.uploadReceiptFiles' } },
  { method: "GET", pattern: /^receipt-file\/([\w-]+)$/, perm: null, handler: async (m, { session, body, url }) => receiptFileService.getReceiptFile(m[1]) },
  { method: "GET", pattern: /^receipt-files\/(\d+)$/, perm: null, handler: async (m, { session, body, url }) => receiptFileService.getReceiptFilesMetadata(Number(m[1])) },
];
