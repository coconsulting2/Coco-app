 ...
 * proceso (React Router v7 framework mode), así que no necesitamos URL absoluta.
 * Si VITE_API_BASE_URL (browser) o API_URL_SSR (server) están definidos los respetamos
 * — útil para apuntar a un backend externo en testing o staging.
 */
...
export function resolveApiBaseUrl(): string {
  ...
  const fromEnv =
    typeof import.meta !== "undefined" && (import.meta as any).env
      ? ((import.meta as any).env.VITE_API_BASE_URL || (import.meta as any).env.PUBLIC_API_BASE_URL)
      : null;
...
interface ApiOptions {
  ...
  // RR v7: callers que necesiten pasar cookies server-side lo hacen con un objeto
  // genérico compatible con Astro.cookies (get/has) o el adapter de getSession.
  cookies?: any;
}
