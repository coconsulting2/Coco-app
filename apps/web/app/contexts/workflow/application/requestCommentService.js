 ...
 * @description Business logic for request comments: create, read, pagination
 * with encrypted cursors. Refactor Fase 6: prisma + withRls extraídos a
 * requestCommentQueries.js. El service ahora orquesta encriptación y paginación.
 */
...
import {
  ...
  listCommentsRls,
  Prisma,
} from "~/contexts/workflow/infrastructure/requestCommentQueries.js";
...
if (!validKey(CHAT_CURSOR_SECRET) || !validKey(CHAT_MESSAGE_SECRET)) {
  ...
  console.warn(
    "[requestCommentService] CHAT_CURSOR_SECRET or CHAT_MESSAGE_SECRET not set — generating ephemeral keys. Only acceptable in development/test.",
  );
...
export async function createComment(userId, requestId, content) {
  ...
  try {
    const [request, user] = await Promise.all([
      findRequestOrgId(requestId),
      findUserOrgId(userId),
    ]);
    ...
    await insertCommentRls(request.organizationId, {
 ...
 * @param {number} userId
 * @param {number} limit
 * @param {{ cursor?: string; suscribe?: boolean }} opts
 */
export async function readComments(requestId, userId, limit, { cursor, suscribe = false }) {
  ...
  const request = await findRequestOrgId(requestId);
  if (!request) {
  ...
  const take = limit + 1;
  const query = { where: { requestId }, take, orderBy: { id: "desc" } };
  ...
  let page;
  try {
    page = await listCommentsRls(request.organizationId, query);
  } catch (err) {
    if (err instanceof Prisma.PrismaClientKnownRequestError && err.code === "P2003") {
  ...
  logger[suscribe ? "trace" : "info"](
    { messagesLength: messages.length, requestId, next: !!result.next },
    "[requestCommentService] readComments",
  );
