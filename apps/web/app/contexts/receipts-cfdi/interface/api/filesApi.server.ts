...
const ROUTES: Array<{ method: string; pattern: RegExp; perm: string | null; handler: (m: RegExpMatchArray, ctx: any) => Promise<unknown> | unknown }> = [
  { method: "POST", pattern: /^upload-receipt-files\/(\d+)$/, perm: null, handler: async (_m, _ctx) => ({ status: 501, message: "multipart upload requires Node adapter — see receiptFileService.uploadReceiptFiles" }) },
