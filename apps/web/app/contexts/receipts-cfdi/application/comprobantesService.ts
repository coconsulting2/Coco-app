 ...
 * @author Hector Lugo
 *
 * Refactor Fase 6: prisma extraído a comprobantesQueries.js.
 */
...
import {
  ...
  acuseToCfdiRow,
} from "~/contexts/receipts-cfdi/infrastructure/satConsultaService.js";
 ...
 */
export async function insertarCfdi(receiptId: any, cfdiData: any) {
 ...
 */
export async function insertarComprobanteInternacional(receiptId: any, body: any) {
  const receipt = await ComprobantesModel.findReceiptById(receiptId);
  if (!receipt) {
    throw { status: 404, message: `Receipt ${receiptId} not found` };
  }
  ...
  const existingCfdi = await findCfdiByReceiptId(receiptId);
  if (existingCfdi) {
  ...
  return upsertReceiptWithCfdiTx({ receiptId, receiptUpdate, cfdiData });
}
