...
const Applicant = {
  /**
   * Busca un Receipt por cfdi_uuid (case-insensitive). Devuelve null si no existe.
   * Centraliza la query que antes vivía inline en applicantService — preserva la
   * regla "solo infrastructure toca Prisma".
   *
   * @param {string} normalizedUuid - UUID en lowercase
   * @returns {Promise<{ receiptId: number }|null>}
   */
  async findReceiptByCfdiUuid(normalizedUuid: string) {
    return prisma.receipt.findFirst({
      where: { cfdiUuid: { equals: normalizedUuid, mode: "insensitive" } },
   ...
   */
  async findCostCenterByUserId(userId: number) {
    const user = await prisma.user.findUnique({
   ...
   */
  async cancelTravelRequest(requestId: number) {
   ...
   */
  async getCompletedRequests(userId: number) {
   ...
   * @returns {Promise<Array<Object>>}
   */
  async getApplicantRequests(userId: number) {
    const requests = await prisma.request.findMany({
      where: {
        userId: Number(userId),
        // 1=Borrador, 8=Finalizado, 9=Cancelado, 10=Rechazado — el dashboard
        // del Solicitante muestra solo solicitudes "activas" (en pipeline).
        requestStatusId: { notIn: [1, 8, 9, 10] },
      },
      include: {
        requestStatus: true,
        routeRequests: {
   ...
   * @returns {Promise<Array<Object>>}
   */
  async getApplicantRequest(requestId: number) {
    const request = await prisma.request.findUnique({
      where: { requestId: Number(requestId) },
      include: {
        user: true,
   ...
   */
  async updateRequestStatusToValidationStage(requestId: number) {
    await prisma.request.update({
      where: { requestId: Number(requestId) },
