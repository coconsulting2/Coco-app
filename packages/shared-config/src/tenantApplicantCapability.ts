...
/** Valores de `Organization.kind` (Prisma) excluidos del merge. */
export const EXCLUDED_ORGANIZATION_KINDS_FOR_APPLICANT_MERGE: ReadonlyArray<string> =
  Object.freeze([]);
...
export type ApplicantCapabilityCtx = {
  userActive: boolean;
  organizationKind?: string | null;
  organizationId?: bigint | number | string | null;
};

export function shouldMergeTenantApplicantCapability({
  userActive,
  organizationKind,
  organizationId,
}: ApplicantCapabilityCtx): boolean {
