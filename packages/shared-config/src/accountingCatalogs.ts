 ...
 */
export const proveedorFromUserId = (userId: number | string): string => {
    const base = 20000000000n + BigInt(Number(userId));
    return base.toString().padStart(11, "0").slice(-11);
};

/** Formatea una fecha como "DD/MM/YYYY" (formato SAP habitual en pólizas). */
export const formatPstngDate = (d: Date): string => {
