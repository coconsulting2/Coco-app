/**
 * @file src/seedHelpers/bootstrapOrganization.ts
 ...
 *
 * El parámetro `includeDittaSuperAdmin` determina qué grupos especiales se incluyen:
 *   - true  (Ditta/ROOT):  grupo DittaSuperAdmin + role Admin Ditta.
 *   - false (CLIENT/resto): grupos default sin DittaSuperAdmin.
 */
import bcrypt from "bcrypt";
import type { PrismaClient, Prisma } from "@prisma/client";
import {
  ACCOUNTING_CATALOG_DEFAULTS,
  TENANT_APPLICANT_CAPABILITY_CODES,
} from "@coco/shared-config";

export { ensureApplicantGroupsForRole } from "#/seedHelpers/applicantRoleGroups.js";
...
type NotificationTemplateSeed = {
  code: string;
  channel: Prisma.NotificationTemplateUncheckedCreateInput["channel"];
  subject: string | null;
  body: string;
};
...
const PERMISSION_GROUPS_DEFAULTS: PermissionGroupSeed[] = [
  {
...
const DITTA_SUPER_ADMIN_GROUP: PermissionGroupSeed = {
...
const DEFAULT_ALERT_MESSAGES: string[] = [
...
const DEFAULT_RECEIPT_TYPES: string[] = [
  "Hospedaje",
  "Comida",
  "Transporte",
  "Caseta",
...
const DEFAULT_NOTIFICATION_TEMPLATES: NotificationTemplateSeed[] = [
  {
...
/**
 * Roles base que cada org recibe. Cada org puede crear roles custom además.
 */
...
export function getDefaultClientRoleNamesForOnboardingImport(): string[] {
  return DEFAULT_ROLES.map((r) => r.roleName);
}

const ROLE_GROUP_ASSIGNMENTS_DEFAULT: Record<string, string[]> = {
 ...
 */
export function getDefaultRolePreviewPermissionCodes(roleName: string): string[] {
 ...
 */
export async function bootstrapOrganizationCatalogs(
  prisma: PrismaClient,
  organizationId: bigint | number,
  opts: { includeDittaSuperAdmin?: boolean } = {},
): Promise<{ rolesByName: Map<string, number>; groupsByName: Map<string, number> }> {
  ...
  const rolesToSeed: RoleSeed[] = includeDittaSuperAdmin
    ? [...DEFAULT_ROLES, { roleName: "Admin Ditta", maxApprovalAmount: null }]
    : DEFAULT_ROLES;

  const rolesByName = new Map<string, number>();
  ...
  const assignments: Record<string, string[]> = { ...ROLE_GROUP_ASSIGNMENTS_DEFAULT };
...
export async function ensureOrganizationAdmin(
  prisma: PrismaClient,
  organizationId: bigint | number,
  params: { email: string; userName: string; password: string; roleName?: string },
): Promise<void> {
  const orgIdBig = BigInt(organizationId);
