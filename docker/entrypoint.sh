...
#  4. Hands off to CMD via exec so the RR v7 server becomes PID 1.
set -euo pipefail

cd /app/certs
if [ ! -f server.crt ] || [ ! -f server.key ] || [ ! -f ca.crt ]; then
  echo "[entrypoint] HTTPS certs missing — generating..."
  # /app/certs may be a named volume that hid the baked-in script + config,
...
echo "[entrypoint] Starting coco-app: $*"
exec "$@"
